...
class WitchesEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if reward == -100:
            ...
            #print("\nGo out of Step with ai_reward:", reward, "Done:", done)
            self.my_game.total_rewards[self.reinfo_index] -=100
            return state.flatten(), -100.0, True, {"number_of_won": self.number_of_won, "correct_moves": self.correct_moves, "total":self.my_game.total_rewards[self.reinfo_index]}
        else:
            rewards, done = self.playUnitlAI()
            state         = self.my_game.getState()
            self.updateTotalResult()
            ...
            return state.flatten(), (dis+21)/26, done, {"number_of_won": self.number_of_won, "correct_moves": self.correct_moves, "total":self.my_game.total_rewards[self.reinfo_index]}
