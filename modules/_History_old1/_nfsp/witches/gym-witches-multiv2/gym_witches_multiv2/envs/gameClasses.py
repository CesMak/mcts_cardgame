...
class deck(object):
    def __init__(self, nu_cards, nu_players, seed=None):
        self.cards      = []
        self.nu_players = nu_players
    ...
    def build(self):
        ...
        for color in ['B', 'G', 'R', 'Y']:
            for val in range(16-int(self.nu_cards/self.nu_players), 16):# choose different deck size here!
...
class game(object):
    def __init__(self, options_dict):
        ...
        self.active_player     = len(self.names_player)-1  # due to gym reset =3 stores which player is active (has to give a card)
        ...
        #Number of cards
        self.nu_cards          = options_dict["nu_cards"] # should be dividable / 4   e.g. 15(maximum), or 4 -> 12,13,14,15 of each color is given.
        self.seed              = options_dict["seed"] # none for not using it

        myDeck = deck(self.nu_cards, self.nu_players, self.seed)
        myDeck.shuffle()
    ...
    def play_ai_move(self, ai_card_idx, print_=False):
        ...
        if player_has_card and tmp in card_options and "RL" in self.player_type[current_player]:
            ...
            return {"state": "play_or_shift", "ai_reward": -100}, False, False # rewards round_finished, game_over, WRONG MOVE -100
            ...
            if "RANDOM" in self.player_type[current_player]:
                if  False:
                    ...
                    if print_:
                        ...
                        print("[{}] {} {}\t shifts {}\tCard {}\tHand Index {}\t len {}".format(self.current_round, current_player, self.names_player[current_player], self.player_type[current_player], self.players[current_player].hand[action], action, len(self.players[current_player].hand)))
                else:
                    hand_idx_action = self.getRandomOption_()
                ...
                rewards, round_finished, gameOver = self.step(hand_idx_action)
                if print_ and round_finished:
                    print(rewards, self.correct_moves, gameOver, "\n")
            else:
                return rewards, round_finished, gameOver
    ...
    def getState(self, player=None):
        ...
        play_options = self.players[player].getBinaryOptions(self.getInColor(), self.nu_players, self.nu_cards, shifting_phase=False)# TODO self.shifting_phase
        #play_options = self.convertAvailableActions(play_options)
        on_table, on_hand, played = self.getmyState(player, self.nu_players, self.nu_cards)
