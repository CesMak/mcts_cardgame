...
class DQNBase(nn.Module):
    ...
    def __init__(self, env):
        ...
        self.input_shape = env.observation_space.shape # (3, 21, 20)
    ...
    def forward(self, x):
        x /= 255.# torch.Size([1, 3, 20, 21])
        x = self.features(x)
        x = self.flatten(x)  # torch.Size([1, 144])
        x = self.fc(x)
        return x

    def _feature_size(self):
        return self.features(torch.zeros(1, *self.input_shape)).view(1, -1).size(1)# 144
