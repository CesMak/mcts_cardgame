...
def learn_single(ppo, update_timestep, eps_decay, env, max_reward=-2):
    memory = Memory()
    ...
    for i_episode in range(1, 500000000+1):
        ...
        while not done:
            ...
            if rewards["ai_reward"] is None: # illegal move
                memory.rewards.append(-100)
            else:#shift round ->0 or leagal play move
                memory.rewards.append(0)
            memory.is_terminals.append(done)
            if info["round_finished"] and rewards["state"] == "play" and done and not rewards["ai_reward"] is None and len(memory.rewards)>5:
                memory.rewards = memory.rewards[:len(memory.rewards)-4]
                memory.rewards.extend(list(rewards["final_rewards"]))
        ...
        if i_episode % log_interval == 0:
            ...
            corr_moves, mean_reward, finished_games =  test_with_random(ppo, env, jjj)
