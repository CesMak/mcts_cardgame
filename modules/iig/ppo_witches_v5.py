...
# Version History:
# Code reference: https://github.com/nikhilbarhate99/PPO-PyTorch/blob/master/PPO.py
...
#               update_step = 160k trembles at
#  _v5      TODO: Include LSTM layer for played

# TODO warum konvergiert am Ende so krass lahm? Dass alle Züge richtig sind?!
# TODO schwankt am Ende wenn konvergiert stark -> step size noch großsser???
# TODO include LSTM??!  -> _v5
# TODO warum wird forward() einmal mit batch size dim und einmal ohne aufgerufen?

# TODO:
# baselines        -> requires tensorflow
# stable-baselines -> requires tensorflow
# rlcard
# rllib   -> nutze das mit dummy state?!
...
class ActorModel(nn.Module):
    def __init__(self, state_dim, action_dim, n_latent_var):
        ...
        self.i_dim   = self.a_dim*3+(state_dim-self.a_dim*4)# dim of info_vector
        self.lstm_hid= 64

        self.ac        = nn.Linear(self.i_dim, n_latent_var)
        self.ac_prelu  = nn.PReLU()
                                 # expected features, hidden state,
        self.ac_lstm   = nn.LSTM(self.a_dim, self.lstm_hid, num_layers=2)

        self.ac1       = nn.Linear(self.lstm_hid+n_latent_var, n_latent_var)
    ...
    def forward(self, input):
        ...
        # Split up the input:
        hand_table = input[:, 0:self.a_dim*2]
        played     = input[:, self.a_dim*2:self.a_dim*3]
        options    = input[:, self.a_dim*3:self.a_dim*4]
        add_states = input[:, self.a_dim*4:input.shape[1]]
        info_vector= torch.cat( [hand_table, options, add_states], 1)

        # Info vector=hand, table, options, add_states
        ac = self.ac(info_vector)
        ac = self.ac_prelu(ac)

        # LSTM played
        # output, h_n, c_n
        # h_n = of shape (num_layers * num_directions, batch, hidden_size)
        output, ([h1_,h2_], [c1_,c2_]) = self.ac_lstm(played.view(batch_size, 1,  played.shape[1]))# h2_.shape = 1,64 (self.lstm_hid)
        print(batch_size, h2_.shape, h1_.shape)
        print(eee)

        # Whole input vector:
        input_vector = torch.cat([ac, h2_], 1)  # 192
        input_vector = self.ac1(input_vector)
        input_vector = self.ac1_prelu(input_vector)

        # Get Actor Result:
        actor_out = torch.cat( [input_vector, options], 1)
        actor_out = self.a1(actor_out)
        actor_out = actor_out.softmax(dim=-1)

        # Get Critic Result:
        critic = self.c1(input_vector)
...
class ActorCritic(nn.Module):
    ...
    def evaluate(self, state, action):
        print("inside evaluate:::", state.shape)
        action_probs, state_value = self.actor_critic(torch.squeeze(state))
        dist = Categorical(action_probs)
...
def playRandomSteps(policy, env, steps, max_corr):
    ...
    for i in range(steps):
        ...
        tmp    = 0
        while not done:
            ttmp = torch.zeros(1, len(state))
            ttmp[0, :] = torch.from_numpy(state).float()
            action = policy.act(ttmp, None)
...
def playSteps(env, policy, steps, max_corr):
    ...
    for i in range(steps):
        ttmp = torch.zeros(1, len(state))
        ttmp[0, :] = torch.from_numpy(state).float()
        action = policy.act(ttmp, memory)# <- state is appended to memory in act function
...
if __name__ == '__main__':
    ...
    gamma           = 0.99
    K               = 16#5
    update_timestep = 100 #2000 # 50000 memory error..... -> mit 35000 nicht so gut (nach 6,5h) wie mit 20000 nach 5h, 20k for 17 moves memory error!
