...
# Note lr=0.01 und 80k funktioniert für den Start ganz gut!  (alles größer lr=0.01 ist zzu viel!)
...
class PPO:
    ...
    def calculate_total_loss(self, state_values, logprobs, old_logprobs, advantage, rewards, dist_entropy):
        ...
        # 4. Total Loss
        beta       = 0.008 # encourage to explore different policies
...
def learn_single(ppo, update_timestep, eps_decay, env, max_reward=-0.1):
    ...
    for _ in range(0, 500000000+1):
        ...
        if i_episode % log_interval == 0:
            ...
            aaa = ('Game ,{:07d}, mean_rew of {} finished g. ,{:0.5}, corr_moves[max:{:2}] ,{:4.4}, mean_rew ,{:1.3},   {},playing t={}, lr={}\n'.format(i_episode, finished_games, float(finished_ai_reward), int(max_corr_moves), float(total_correct), float(total_ai_reward), datetime.datetime.now()-start_time, playing_time, ppo.scheduler.get_lr()))
            print(aaa)
