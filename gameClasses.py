...
class player(object):
    ...
    def indexToAbsolute(self, index):
        # convert to absolute index (option possible)
        card = self.hand[index]
        return self.getCardIndex(card)
        ...
        # return card index of the incard of the hand
            for i, card in enumerate(self.hand):
                if card.color == incard.color and card.value == incard.value:
                    return i
            print("Error? in specific Index Hand!")
            return 0 # error!
...
class game(object):
    ...
    def getBinaryState_(self, playeridx, action, bestq):
        ...
        return [bin_options+bin_on_table+bin_played]+[0,0]
    ...
    def NNStep(self):
        'requires to import testing'
        incolor = None
        # play the card:
        if len(self.on_table_cards)>0:
            incolor = self.on_table_cards[0].color

        line = self.getBinaryState_(self.active_player, 0, -1.0)
        action = testing(line)# action from 0-60 -> transform to players action!
        played_card   = self.players[self.active_player].getIndexOfCard(action)
        action        = self.players[self.active_player].specificIndexHand(played_card)

        is_allowed_list_idx = self.getValidOptions(self.active_player)
        if action not in is_allowed_list_idx:# if NN step not allowed do random step
            played_card = self.players[self.active_player].playRandomCard(incolor)
        else: # NN Step:
            played_card = self.players[self.active_player].hand.pop(action)
